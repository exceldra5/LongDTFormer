...
from utils.DataLoader_role import get_idx_data_loader, get_link_prediction_data_with_roles, get_role_idx_data_loader
    ...
    for run in range(args.num_runs):
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                # get embedding of same role nodes
                # two Tensors, with shape (batch_size, node_feat_dim)
                batch_src_node_embeddings_for_role, batch_src_same_role_node_embeddings_for_role = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids_for_role,
                                                                      dst_node_ids=batch_src_same_role_node_ids_for_role,
                                                                      node_interact_times=train_data.node_interact_times[selected_src_node_indices_for_role],
                                                                      snapshots=batch_src_node_snapshots_for_role)
                # NOTE(wsgwak): node_interaction_times -> How to set for role sampling?
                ...
                positive_probabilities_for_role = model[1](input_1=batch_src_node_embeddings_for_role,
                                                            input_2=batch_src_same_role_node_embeddings_for_role).squeeze(dim=-1).sigmoid()
                
                # role aware loss
                predicts = torch.cat([positive_probabilities, positive_probabilities_for_role, negative_probabilities], dim=0)
                labels = torch.cat([torch.ones_like(positive_probabilities), torch.ones_like(positive_probabilities_for_role), torch.zeros_like(negative_probabilities)],
                                   dim=0)
